...
// Set this once the Cloudflare Worker is live (Part C deploy).
// Until then, submitEmail writes state locally and reports success so the
// UI is testable.
...
export const COPY = {
  ...
  welcome: {
    ...
    note: 'Sentry also auto-detects sign-ups when you visit checkout pages on supported services.'
  },
...
export async function shouldShowWaitlistPrompt(triggerSource) {
  ...
  // 4. Lifetime exposure cap
  if (s.exposures.length >= MAX_EXPOSURES) return false;
